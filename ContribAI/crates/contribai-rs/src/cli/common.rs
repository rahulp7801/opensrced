...
/// Create an independent reviewer LLM provider, if `reviewer` is configured.
/// Falls back to `None` (caller should use the primary `llm`) when unset.
pub fn create_reviewer_llm(
    config: &ContribAIConfig,
) -> anyhow::Result<Option<Box<dyn contribai::llm::provider::LlmProvider>>> {
    match &config.reviewer {
        Some(rev) => contribai::llm::provider::create_llm_provider(rev)
            .map(Some)
            .map_err(|e| anyhow::anyhow!("{}", e)),
        None => Ok(None),
    }
