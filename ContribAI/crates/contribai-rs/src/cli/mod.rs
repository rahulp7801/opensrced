...
enum Commands {
    ...
    Solve {
        ...
        /// Solve only the specified issue number (skip classification filter).
        #[arg(long)]
        issue: Option<u64>,
    },
...
impl Cli {
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        match command {
            ...
            Commands::Solve { url, dry_run, issue } => {
                commands::solve::run_solve(self.config.as_deref(), url, dry_run, issue).await
            }
            Commands::McpServer => {
