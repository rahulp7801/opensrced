...
pub struct ContribAIConfig {
    ...
    #[serde(default)]
    pub llm: LlmConfig,
    /// Optional independent LLM used as the self-review critic. Pointing this
    /// at a different model (e.g. a non-coder local model, Gemini, etc.) is
    /// far more reliable than asking the same code-gen model to grade its own
    /// output. When unset, self-review uses the primary `llm` config.
    #[serde(default)]
    pub reviewer: Option<LlmConfig>,
    #[serde(default)]
