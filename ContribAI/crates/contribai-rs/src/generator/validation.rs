...
impl ContributionGenerator<'_> {
    ...
    pub(crate) fn validate_changes(&self, changes: &[FileChange]) -> bool {
        ...
        for change in changes {
            ...
            // Balanced bracket check (string/comment-aware) — only meaningful
            // for actual source code. Markdown, plaintext, and config dialects
            // routinely contain unbalanced punctuation in prose or examples.
            let is_code = !matches!(
                change
                    .path
                    .rsplit('.')
                    ...
                    .as_deref(),
                Some("md" | "markdown" | "rst" | "txt" | "csv" | "tsv" | "log")
            );
            if is_code && !content.is_empty() {
