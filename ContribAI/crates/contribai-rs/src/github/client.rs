...
impl GitHubClient {
    ...
    /// Search a single repository for code containing `query`. Returns the
    /// matching file paths (deduped). Used to localize symbol mentions
    /// from issue bodies (function/class names) to actual files.
    pub async fn search_code_in_repo(
        &self,
        owner: &str,
        repo: &str,
        query: &str,
    ...
    pub async fn create_pull_request(
        ...
        // CONTRIBAI_DRAFT_PR=1 makes every opened PR a draft. Defaults off.
        // Truthy: "1", "true" (case-insensitive). The dispatcher sets this
        // by default so dashboard-approved PRs land as drafts the user can
        // review/edit before marking ready.
        let want_draft = std::env::var("CONTRIBAI_DRAFT_PR")
