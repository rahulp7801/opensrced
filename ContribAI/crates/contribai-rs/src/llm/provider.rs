...
/// Process-wide rate limiter for Gemini API-key calls, to stay under the
/// free-tier 20 RPM limit. All GeminiProvider instances share the same
/// "next-allowed-slot" Instant. Configurable via CONTRIBAI_GEMINI_RPM
/// (default 15 — one call every 4 s, safety margin vs the 20 RPM cap).
static GEMINI_NEXT_SLOT: std::sync::Mutex<Option<std::time::Instant>> =
    std::sync::Mutex::new(None);

async fn gemini_throttle() {
    let rpm: u64 = std::env::var("CONTRIBAI_GEMINI_RPM")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(15)
        .max(1);
    let min_interval = std::time::Duration::from_millis(60_000 / rpm);

    let sleep_until = {
        let mut guard = GEMINI_NEXT_SLOT.lock().unwrap();
        let now = std::time::Instant::now();
        let slot = match *guard {
            Some(next) if next > now => next,
            _ => now,
        };
        *guard = Some(slot + min_interval);
        slot
    };

    let wait = sleep_until.saturating_duration_since(std::time::Instant::now());
    if !wait.is_zero() {
...
impl OllamaProvider {
    pub fn new(config: &LlmConfig) -> Result<Self> {
        ...
        // 600s (10min) — local models on partial-GPU/CPU can take several minutes
        // per generation, especially on warm-up. Remote providers never need this long.
        let timeout_secs: u64 = std::env::var("CONTRIBAI_OLLAMA_TIMEOUT_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(600);
...
impl LlmProvider for OllamaProvider {
    ...
    async fn chat(
        ...
        let temp = temperature.unwrap_or(self.temperature);
        // Cap output length — without num_predict, Ollama lets the model run
        // until it emits a stop token, which some code models never do
        // (they fall into repetition loops and grind for 20+ min). The cap
        // makes each call finite-time.
        let num_predict = max_tokens.unwrap_or(self.max_tokens) as i32;
